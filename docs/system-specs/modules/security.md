...
- Sensitive paths include host credential stores (`~/.aws`, `~/.ssh`, `~/.gnupg`, `~/.gpg`, `~/.config/gcloud`, `~/.azure`, Docker/Kubernetes/npm/PyPI/netrc/git credentials), vendor ACP credentials, and Kiro Crew's secret and governance roots. Legacy root-level PID authority leaves remain protected even though current process tracking lives under `run/`.
- **Crew data-home secret/trust-root leaves are covered under EVERY known home prefix.** Every entry in `_CREW_SECRET_LEAVES` is expanded under both `_CREW_HOME_PREFIXES = (".kiro/crew", ".kirocrew")`, covering current and deprecated homes with one source of truth. This includes `.env`, BYOK and browser credentials, SEL and trust material, `security_policy.json`, `profiles/`, `policy_cache/`, `admission_policy.json`, `computer_use.json`, cron and workflow state, OAuth and signing keys, routing, `run/`, `run-coordinator/`, and app-owned credential/trust roots. New keystone state is added once and inherits both prefixes. The migration force-deletes `~/.kirocrew` once the move completes — there is no rollback copy to gate.
- **Run coordinator ledger.** `run-coordinator/` contains canonical run, command, lease, and delivery metadata plus SQLite WAL/SHM/journal sidecars. Protecting the whole directory prevents an agent from reading task payloads, forging completion delivery, or rewriting desired/observed state and fencing metadata. Every OS-sandbox mode also masks the active relocated ledger when `KIROCREW_HOME` lies outside the default home; gateway coordinator code opens it directly. Files and directories are owner-only on creation and reopen.
...
- **An empty-elided render is ADDED as a third view, never substituted for the plain join (issue #7500).** An empty-quoted word — `""`, `''`, `$''`, `$""`, or any concatenation of them (`""''`, `""""`) — is a real argv element that the shell does hand to the program, so `_shell_tokens` is correct to keep it and the payload walk still reads argv as written. What it cannot survive is the RENDER: a single-space join turns a zero-width element into a spurious extra separator, so `rm -rf "" /home/x` rendered as `rm -rf  /home/x` and the `rm -rf /.*` rule stopped matching its own target — the destructive command ran exactly as written and only the gate was fooled. The escape was pattern-DEPENDENT, not systematic: `chmod "" 777 /etc/passwd` stayed denied because the rule that catches it (`chmod.*/etc/.*`) tolerates the extra separator, which is why the repair belongs in the render and not in individual rules. **Both whitespace shapes are emitted**, because REPLACING the plain join loses denials that existed: a rule that REQUIRES an intervening token (`rm -rf .* ./data`) matched the double-spaced view and matches neither the elided one nor the command's canonical spelling, so `r""m -rf "" ./data` was refused before and became allowed — the additive-only property this helper documents is load-bearing, not tidiness (found by the GPT 5.6 lane, reproduced against the merge-base; pinned by `test_the_elided_view_is_added_and_never_substituted`). The fix is in the RECOGNIZER: `_shell_tokens`' documented contract — argv as a POSIX shell hands it over — is unchanged, so the path normalizer's ~19 consumers see identical tokens. The six self-protection rules were never fooled, because their argv-structural floor does not match a rendered line. Pinned by `TestEmptyArgvElementDoesNotBreakTheDenyView`.
  - **A shell call whose arguments have not streamed yet is deferred, not admitted.**
    `is_shell and not command` still denies, and still has no override — but the
    ACP layer no longer PRESENTS that state when the arguments are merely late.
    `_dispatch_events` parks such a permission request until the refinement
    carrying them lands, then gates the rebuilt event on real bytes; a request
    whose arguments never arrive is flushed unverified and denied exactly as
    before. This admits no command the gate did not read, which is the property
    the refusal exists to hold; see acp-client.md for the mechanism and its caps.
