...
Each serialized slot also carries `provider_label`, resolved from its live
provider or persisted `SessionMap` entry using the effective session key
(`linked_session_key` or `dashboard:{slot.key}`). Legacy Kiro entries normalize
to `acp`; a new unbound slot remains unlabeled until its first provider starts.
The composer renders this persisted identity, so changing the global ACP backend
does not relabel an existing session.
