...
| **Copilot** | `ACP_BACKEND_COPILOT = "copilot"` — GitHub Copilot CLI's own `copilot --acp` server. This build's adapter; upstream ships none. Cells read `—` until a live corpus is captured (see `.fork-sync.yml`, divergences/fork-adapters-without-corpus). |
| **goose** | `ACP_BACKEND_GOOSE = "goose"` — `goose acp`, its own binary. This build's adapter, known but not selectable (no install probe). Cells read `—` for the same reason. |
| **pi** | `ACP_BACKEND_PI = "pi"` — the `pi-acp` registry adapter. This build's adapter, known but not selectable (no install probe). Cells read `—` for the same reason. |

## 1. Agent definition and layout

| | kiro-cli | KAS | CC | Codex | OpenCode | Copilot | goose | pi |
|---|---|---|---|---|---|---|---|---|
