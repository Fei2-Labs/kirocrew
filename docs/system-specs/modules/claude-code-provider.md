...
`KiroCrewConfig.create_provider_factory()` constructs an ACP provider. Harness
choice is a separate field, `agent.acp_backend`; `kiro-cli` remains the default.
The public baseline selects Kiro, Claude, KAS, Codex, OpenCode, Pi, goose, and
this fork's additional GitHub Copilot adapter. DeepSeek is known but withheld:
its own sandbox handles ordinary tool calls without routing them through Crew's
host permission gate. Tests pin the selectable set and this one explicit
exception.
