...
- `"opencode"` (`ACP_BACKEND_OPENCODE`): `opencode acp` (resolved by `_resolve_opencode_bin` — `OPENCODE_ACP_BIN` env override, then augmented PATH, then the install-script default `~/.opencode/bin/opencode`). This is the BYOK seam: OpenCode serves any OpenAI-compatible endpoint configured in `~/.config/opencode/opencode.json`, advertising each as a `provider/model` id in `session/new`'s standard `models` payload (`{currentModelId, availableModels:[{modelId,name}]}`), and honours `session/set_model`. Numeric protocolVersion (1), loadSession=true, one process per session on `AcpClient`, plain `session/load` with no `_meta`.
