...
- `"copilot"` (`ACP_BACKEND_COPILOT`): `copilot --acp` (resolved by `_resolve_copilot_bin` — `COPILOT_ACP_BIN` env override, then augmented PATH, then the per-user `github-copilot-sdk/cli/<version>/` install dir). Speaks the numeric protocolVersion (1) like claude-agent-acp; one process per session on `AcpClient`; plain ACP `session/load` keyed by sessionId, no `_meta`.
- `"opencode"` (`ACP_BACKEND_OPENCODE`): `opencode acp` (resolved by `_resolve_opencode_bin` — `OPENCODE_ACP_BIN` env override, then augmented PATH, then the install-script default `~/.opencode/bin/opencode`). This is the BYOK seam: OpenCode serves any OpenAI-compatible endpoint configured in `~/.config/opencode/opencode.json`, advertising `provider/model` ids in `session/new`'s standard `models` payload (`{currentModelId, availableModels:[{modelId,name}]}`), and honours `session/set_model`. Some OpenCode ACP versions advertise only the current model, so Kiro Crew safely supplements that list with enabled `provider.models` entries from the operator-owned config: only syntax-valid ids are read, credential fields are never read or returned, ACP-advertised rows win, and malformed config degrades to the ACP list. Numeric protocolVersion (1), loadSession=true, one process per session on `AcpClient`, plain `session/load` with no `_meta`.
...
- The dashboard `/api/models` one-shot subprocess validates completion before parsing stdout: nonzero exit (with a bounded, redacted stderr tail), empty stdout, malformed JSON, or a payload without a model list each returns HTTP 503 so the client retries. A subprocess failure is never misreported as `JSONDecodeError` or cached as a successful empty model list. For a selected non-Kiro backend, the endpoint reads the newest live provider with that exact positive backend identity, never the first active provider: ACP backends advertise independent model namespaces, so a stale Kiro or Copilot session cannot hide an OpenCode `provider/model` BYOK id from the picker. Claude alone maps its advertised ids through the registry; other backend ids remain wire values.
