...
| H12 | Model pre-flight keeps "empty or unknown advertised set means allow", and never compares ids across harness namespaces. Harnesses advertise ids in their own spelling; one shared membership test across two namespaces calls every legitimate model unusable and withholds the model. Which backends the pre-flight applies to at all is opt-in membership in `ACP_BACKENDS_MODEL_NAMESPACE` (kiro, kas, copilot, opencode): backends whose advertised ids are exactly the ids their `session/set_model` accepts. The claude seam is excluded (bare advertised ids vs prefixed configured ids). | `test_harness_parity.py::test_model_preflight_allows_unknown_advertised_set`, `::test_model_namespace_membership` | `acp/client.py` (`model_is_unusable`, `advertised_model_ids`, `_model_namespace_preflight`), `acp/types.py` (`ACP_BACKENDS_MODEL_NAMESPACE`) |
