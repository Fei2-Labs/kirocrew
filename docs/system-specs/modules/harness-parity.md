...
exception — pinned by
`test_agent_backend_editable.py::test_baseline_ships_the_reviewed_backends_only`, which
...
4. A new harness adds rows to `ACP_BACKENDS_KNOWN`, a `PROVIDER_LABEL_*`, and
   ...
   NARROWING that `test_baseline_ships_the_reviewed_backends_only` fails on **unless**
