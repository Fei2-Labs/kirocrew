...
## Upstream reference: Claude described as a selectable harness

Upstream documents this harness as publicly selectable. This fork withholds
selection (see **Public provider boundary** above, which governs): an adapter
whose tool calls Kiro Crew cannot govern is refused by default with one named
opt-out, and adapters stay operator-installed rather than bundled. The material
below is retained verbatim because it specifies real adapter behaviour — settings
routing, the MCP array, the governance and probe boundaries, and the known gaps —
that an edition enabling the seam must honour. Where it asserts that
`ACP_BACKEND_CLAUDE` ships in `BASELINE_SELECTABLE_BACKENDS`, this fork's
boundary section is authoritative instead.
...
per-entry `claude_code` provider IDs are registry mappings for this adapter's
