...
def _pid_start_token(pid: int) -> str | None:
    ...
    Delegates to :func:`platform_compat.get_process_start_id`, the shared
    ...
    """
    # ``get_process_start_id``, not ``process_start_time``: the other NINE call
    # sites in this file use it, upstream uses it, and both helpers read the same
    # kernel field -- the fork spelling survived at this ONE site through the
    # merge. A split spelling is not cosmetic here: a test (or any caller) that
    # substitutes the identity seam patches the name the rest of the file uses,
    # so this site fell through to a real /proc read and a real PID-file read.
...
_MANAGED_AGENT_MARKERS: tuple[str, ...] = (
    ...
    "pi",
    "copilot",
    # deepseek's harness binary (``dsh --profile acp``). Present so a deepseek
    # child is reapable rather than a leaked orphan; EXACT-matched below, since
    # a three-letter substring needle would claim unrelated processes.
    "dsh",
)
...
_MANAGED_AGENT_EXACT_MARKERS = frozenset(
    {"claude", "codex", "goose", "opencode", "pi", "copilot", "dsh"}
)
...
# The fork's own marker-split detector lived here until the 2026-09-21 sync.
# Upstream REPLACED it with the tokenized ``_is_managed_agent_process`` defined
# below, and that one is strictly stronger: it compares argv0 BASENAMES exactly
# against the registry-projected set instead of running three-character needles
# as substrings over a whole command line, where ``goose`` matches ``mongoose``.
# The marker tuples above are retained: they still carry the authorization rule
# (only code-owned identities may authorize the legacy fallback -- registry
# metadata is operator/agent-writable, so a package string such as "python"
# there would let a forged PID entry target an unrelated same-user process) and
# ``_MANAGED_AGENT_MARKERS`` is what the parity tests read.
    ...
    for line in lines:
        ...
        try:
            ...
            total_killed, root_killed = _kill_pid_tree(
                pid,
                expected_start_token=recorded_token if token_settled else None,
            )
            killed += total_killed
            if root_killed:
                killed_or_dead.add(stripped)
...
def _kill_confirmed_and_writeback(
    ...
    orphan_killed = 0
    for pid, candidate_token in confirmed:
        found = index.get(pid)
        if found is None:
        ...
        if recorded_token is not None:
            ...
            if live_token != recorded_token:
                ...
                continue
        total, root = _kill_pid_tree(pid, expected_start_token=recorded_token)
        orphan_killed += total
