...
class AcpWorker(Worker):
    ...
    async def start(self) -> None:
        ...
        if acp_backend == ACP_BACKEND_KAS:
            ...
            if requires_spec_adapter_admission(acp_backend):
                self._client = SpecAdapterAcpClient(
                    agent=AGENT_NAME,
                    sandbox_mode=sandbox_mode,
                ...
                # `requires_spec_adapter_admission` is False here only for a
                # kiro-dialect id (KAS took its own branch above), so this
                # construction is always the kiro default -- no `acp_backend`
                # to pass (test_the_knowledge_pool_client_takes_the_default_backend
                # pins the direct `AcpClient(...)` call to name none).
