...
def _type_change_refusal(rel: str) -> str:
    """The refusal text for an approved file that is now a directory.

    Shared because the type change surfaces as two different errnos: EISDIR on
    Linux (``IsADirectoryError``) and EPERM on macOS (``PermissionError``).
    """
    return f"refusing to discard {rel!r}: it is now a directory, not the file that was confirmed"
    ...
    for rel in rel_paths:
        ...
        try:
            try:
                ...
                return _type_change_refusal(rel)
            except PermissionError:
                # macOS answers EPERM (PermissionError) rather than EISDIR when
                # `unlink` is handed a directory, so the type change would
                # otherwise be reported as a generic permission failure and the
                # caller would never learn WHAT changed. Confirm the type
                # against the already-pinned directory fd -- never by path,
                # which would re-resolve the very ancestors this walk exists to
                # pin -- and only then name the type change.
                try:
                    swapped_to_dir = stat.S_ISDIR(
                        os.stat(parts[-1], dir_fd=dir_fds[-1], follow_symlinks=False).st_mode
                    )
                except OSError:
                    swapped_to_dir = False
                if swapped_to_dir:
                    return _type_change_refusal(rel)
                return f"could not discard {rel!r}: permission denied"
