...
# Exported for the anti-drift test: every persisted caller-supplied string
# field that the chokepoint validates.
CRON_VALIDATED_STRING_FIELDS: frozenset[str] = frozenset(
    name for name, _ in _CRON_STRING_FIELD_CAPS
)
...
def _validate_cron_string_fields(
    ...
    *,
    ...
    - Otherwise: validates only when truthy (falsy = "not set", a no-op).
    """
