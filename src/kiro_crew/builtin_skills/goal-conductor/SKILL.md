...
5. `session_ledger_record` the item: its goal text, the round number, and — in
   ...
   acceptance spec, session key, round, status and read cursor (see "What goes
   where" below for the encoding and its bounds). That entry is the ONLY place
   the acceptance spec survives compaction; `next` carries the resumable intent.
...
2. **Evaluate every open item's acceptance condition with the bundled
   ...
   **Resolve `<this skill's dir>` from where this SKILL.md was actually loaded
   ...
   allowlist — rewrite the condition around an allowlisted verdict-carrier
   (`pytest`, `gh`, `git`, `npm`, `make`, `cargo`, `go`) or surface it to the
   user; never retry around the list. `error` is a broken spec or environment —
   fix the spec or ask.
...
- `artifacts` — **the durable home for every active work item.** One entry per
  ...
  Key it `item-<n>`; the value is one compact JSON object:

  ```
  item-1 -> {"accept":{"kind":"pr_checks","pr":123,"repo":"o/r"},
             "session":"<session key>","round":2,"status":"running",
             "since":"<next_since cursor>"}
  ```

  The field's real bounds make this fit and also bound it: **32 entries, key
  ≤128 chars, value ≤2000 chars** — ample for one item's spec, far too small for
  prose or a transcript excerpt. So keep values to this encoding, and **rotate**:
  when an item reaches a terminal verdict, collapse its entry to a one-line
  outcome (`{"status":"pass","round":2}`) or drop it, so a long goal's finished
  items cannot age an ACTIVE item out of the 32-entry cap. Record the entry in
  the same `session_ledger_record` call that marks the item dispatched, and
  rewrite it whenever the spec concretizes (the two-phase TBD case) or the cursor
  advances.
...
- **Shell is for the evaluator only, and the allowlist is not a sandbox.**
  `execute_bash` exists so patrol can run `accept_eval.py`; every call is
  audit-logged and every call prompts. The evaluator's `ALLOWED_COMMANDS` holds
  only verdict-carrying test/build/query tools — no bare interpreter (`python`,
  `node`) and no arbitrary-package runner (`npx`) — so a spec cannot name
  `python -c <payload>` and have it run. But `make`, `npm`, `cargo` and `go`
  still execute project-authored code (a Makefile recipe, an npm script, a
  `build.rs`), so the allowlist NARROWS what an acceptance spec can reach; it
  does not make a hostile spec safe. The control that actually bounds execution
  is the per-call approval on `execute_bash`. Treat a `refused` verdict as a
  spec to rewrite, never a list to route around.
