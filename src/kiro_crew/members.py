...
def select_provider_backend(
    ...
    Every arm is narrowed to a KIRO-dialect id by
    :func:`_kiro_dialect_only`. The caller is the DIRECT Kiro factory, which
    constructs ``AcpProvider`` unconditionally; a SPEC-dialect id reaching it
    would give a registry adapter the Kiro provider contract instead of
    ``SpecAdapterAcpProvider`` — the catch-all inheritance H5 forbids. Two
    SPEC backends (``copilot``, ``opencode``) are selectable on a plain public
    build, so this is not a hypothetical. The dialect-correct route for those
    is the registry dispatcher reached through
    ``PROVIDER_BACKEND_FACTORY_ATTR``, which ``session_allocation`` already
    swaps in whenever the requested backend differs from the configured
    snapshot.
    """
    if explicit:
        return _kiro_dialect_only(_resolve(explicit), "explicit caller pick")
    if is_member_session_key(session_key):
        backend = _kiro_dialect_only(
...
def _kiro_dialect_only(backend: str, source: str) -> str:
    """Degrade a SPEC-dialect id to Kiro with a reason, never raise.
    ...
    if backend in ACP_BACKENDS_KIRO_DIALECT:
        return backend
    logger.info(
