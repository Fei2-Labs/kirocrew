...
def select_provider_backend(
    ...
    Every arm crosses the same gate. The caller is the DIRECT Kiro factory, which
    constructs ``AcpProvider`` unconditionally: upstream's harnesses (kiro, KAS,
    claude, codex, opencode) are all served by it, so they pass through. Only
    this build's own registry adapters (``ACP_BACKENDS_REGISTRY_ADAPTERS``) are
    degraded to kiro here -- their dialect-correct route is the registry
    dispatcher reached through ``PROVIDER_BACKEND_FACTORY_ATTR``, which
    ``session_allocation`` swaps in whenever the requested backend differs from
    the configured snapshot.
    """
    if explicit:
        return _direct_factory_backend(_resolve(explicit), "explicit caller pick")
    if is_member_session_key(session_key):
        backend = _direct_factory_backend(
            _resolve(member_backend), f"agent.member_acp_backend for {session_key!r}"
        )
        logger.info(
        ...
        return backend
    return _direct_factory_backend(configured_default, "configured default")


def _resolve(backend: str) -> str:
    """Cross the shared selectability/governance gate (H3's single gate)."""
    from kiro_crew.acp_backends import resolve_selected_backend

    return resolve_selected_backend(backend)


def _direct_factory_backend(backend: str, source: str) -> str:
    """Degrade one of this build's registry adapters to Kiro with a reason.

    Degrading rather than raising is H3's contract for a backend this path
    cannot serve: the member thread runs as plain Kiro chat and the log says
    why, instead of a start-up stack trace on a value the operator was allowed
    to configure. Upstream's own harnesses are not touched.
    """
    from kiro_crew.acp_backends import ACP_BACKEND_KIRO, ACP_BACKENDS_REGISTRY_ADAPTERS

    if backend not in ACP_BACKENDS_REGISTRY_ADAPTERS:
        return backend
    logger.info(
        "acp_backend %r (%s) is a registry-adapter dialect and cannot be served by "
        "the direct Kiro provider factory; using the Kiro harness instead",
        backend,
        source,
    )
