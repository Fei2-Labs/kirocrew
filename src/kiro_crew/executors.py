...
# Bounded SQLite run-coordinator bulkhead pool. Fork-only (run_coordinator is
# being retired in favor of taskq -- see the fork-sync task doc), kept so the
# coordinator's own module still imports until that retirement is complete.
...
def shutdown_maintenance_executor() -> None:
    ...
    global _crew_log_pool, _kiro_spawn_pool, _coordinator_pool
    with _lock:
