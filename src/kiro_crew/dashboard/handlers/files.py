...
async def api_project_git_status(request: web.Request) -> web.Response:
    ...
    seen_keys: set[tuple[str, str | None, str | None, bool | None]] = set()
    for f in result.get("files", []):
        f["path"] = redact(f["path"])
        if f.get("repoRoot"):
            f["repoRoot"] = redact(f["repoRoot"])
        # Dedup key is built AFTER the redactions above, so two genuinely
        # different paths that redact to the same placeholder collapse here.
        # ``repoRoot`` is part of the key because the row list is FLAT across a
        # multi-repo workspace: the same relative path exists once per repo, and
        # keying without the root collapsed those siblings into a single row —
        # the discovery this endpoint exists to report. Two rows still collapse
        # when their roots also redact alike, which is the intended case: they
        # are then indistinguishable to the reader the redaction is for.
        dedup_key = (f["path"], f.get("repoRoot"), f.get("status"), f.get("staged"))
