...
def install_snapshot() -> Dict[str, Dict[str, Any]]:
    """The rows of :func:`_snapshot`, keyed by backend id. BLOCKING -- offload it.

    This build merges the machine half into the registry descriptors served by
    :mod:`kiro_crew.dashboard.handlers.acp_backends` (the one registration of
    ``/api/acp-backends``), so it wants a lookup by id rather than a sorted list.
    Same rows, same owner, no second probe.
    """
    return {row["id"]: row for row in _snapshot()}
