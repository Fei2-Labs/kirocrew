...
# ── Fork addition: the one PRE-SESSION question that is not a live-session field ──
# The record above answers questions ABOUT A SESSION. This one is asked in order to
# decide how to BUILD one, so no value read off a live session can answer it, and it
# stays a named predicate rather than becoming a further field. Computed per call,
# never frozen at import: a backend's descriptor can arrive from the ACP registry
# cache after this module is imported, so an answer cached at import time would be a
# different answer than the spawn's. Imported inside the function because the driver
# pulls in the ACP layer, which this module is deliberately above.
