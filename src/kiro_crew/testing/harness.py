...
def _end_escaped_descendants(escaped: dict[int, str]) -> bool:
    ...
    deadline = time.monotonic() + GROUP_EXIT_WAIT_SECONDS
    while True:
        # A pid whose identity can no longer be READ but still exists is a zombie
        # awaiting its reaper (macOS libproc answers None for one): still ours,
        # still counted, until it is gone or the deadline passes. Only a pid whose
        # identity reads as a DIFFERENT process is a recycled stranger.
        alive = [
            child
            for child, token in escaped.items()
            if platform_compat.pid_exists(child)
            and platform_compat.process_start_time(child) in (None, token)
        ]
        if not alive:
            return True
        if time.monotonic() >= deadline:
