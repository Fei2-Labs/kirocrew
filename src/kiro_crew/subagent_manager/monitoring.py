...
class OrphanStallMonitor(ManagerComponent):
    ...
    async def _reconcile_orphans_impl(self) -> None:
        ...
        """
        # Durable completions are drained FIRST: an event still pending in the
        # outbox owns its run's delivery, and a folder scan that tombstoned it
        # as an orphan would take that ownership away from the one path that
    ...
    async def _reaper_loop_impl(self) -> None:
        ...
        while True:
            ...
            now = time.time()
            # Periodic durable recovery: expired leases are fenced and their
            # committed completions re-delivered. Live and locally queued runs
            # are excluded so recovery cannot race an owner in this process.
            try:
