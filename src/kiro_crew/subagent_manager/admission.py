...
class SpawnAdmissionCoordinator(ManagerComponent):
    ...
    def spawn_impl(
        ...
        _coordinator_command: Any | None = None,
        _coordinator_fence: Any | None = None,
        ...
        if should_queue:
            ...
            info = SubagentInfo(
                ...
                include_project=include_project,
                _coordinator_admitted=_coordinator_admitted,
                # A queued admitted run has not started, so its command is
                # still waiting: committing it here would apply work that may
                # yet be cancelled out of the queue.
        ...
        # stagger gate: one start per interval
        ...
        try:
            # Parent session trust (approval_policy="auto"), set by the
            # dashboard trust toggle.
            parent_trusted = (
                parent_session_key
        ...
        if yolo_enabled:
            ...
            if self._manager._ctx_builder.hooks.auto_approve_subagent_spawn is True:
                ...
                # `spawn` returns before the human answers, so an admitted run
                # awaiting approval is a WAITING execution: its command must
                # stay uncommitted until the answer arrives, and the rejection
                # path needs a waiting claim to finish durably.
            ...
            # See above: approval-pending admitted work stays a waiting
    ...
    def _announce_rejection_impl(
        ...
        *,
        ...
        coordinator_command: Any | None = None,
    ...
    def _drain_queue_impl(self) -> None:
        ...
        try:
            ...
            # A drained spawn has no caller to raise into: this is a timer
            # callback. Synthesize the terminal record so the rejection is
            # reported instead of leaving the run "running" forever.
            spawn_raised = True
        ...
        if legacy_coordinator_rejection and drained is not None:
            # The original caller returned when this entry was queued. If
            # revalidation now rejects it, no authority call remains on the
    ...
    async def _spawn_with_approval_impl(self, info: SubagentInfo) -> None:
        ...
        if not approved:
            ...
            if info._coordinator_fence is not None:
                try:
                    # Commit the rejection BEFORE any local terminal marker, and
                    # keep the heartbeat renewing: the run lease is what protects
                    # terminal delivery until the report below has landed.
                    await self._manager.command_authority.reject_waiting_execution(
                        info.id,
                        rejection_error,
            ...
            if self._manager._claim_finalize(info):
                if info._coordinator_fence is not None:
                    # A fenced rejection is a wave member: it needs the terminal
                    # report path so its digest hold is settled, not a bare
                    # announce that leaves the wave short one member.
                    await self._manager._report_terminal(
                        info,
