...
class ContinuationCoordinator(ManagerComponent):
    ...
    def _conversation_busy_impl(self, conv_key: str) -> SubagentInfo | None:
        ...
        # and a continuation admitted in that window resumes onto session
        # files that are about to be deleted underneath it.
    ...
    async def steer_run_impl(self, agent_id: str, message: str) -> tuple[bool, str]:
        ...
        if not getattr(provider, "supports_steer", False):
            # Steer is an opt-in capability (membership in ACP_BACKENDS_STEER,
            # surfaced as `supports_steer`), never inherited -- H6. A harness
            # outside it has no ``_session/steer``: the notification is
            # fire-and-forget, so sending it LOOKS like success and then hangs
            # or no-ops. Queue the correction as a follow-up instead, and name
            # the harness in the detail so the caller knows why.
            label = _harness_label(provider)
            ok, detail = await self._manager.follow_up_run(agent_id, message)
            if ok:
                return True, (
    ...
    def _prepare_conversation_release_impl(
        ...
        Runs entirely on the event loop: the session registry and the TTL map
        are loop-affine, so the mutation cannot be moved into the worker
        thread that does the file cleanup. The returned tuple is that
        thread's whole input.
        """
        ...
        # Held until the cleanup below finishes, so `_conversation_busy`
        # refuses a continuation for the whole window in which the files
        # still exist but the registry no longer claims them.
    ...
    def _finish_conversation_release_impl(
        ...
        Blocking: both the state write and the file removal are synchronous
        filesystem work, so the async caller runs this off the event loop.
        """
    ...
    def release_conversation_impl(self, conv_id: str) -> tuple[bool, str]:
        ...
        result, cleanup = self._prepare_conversation_release_impl(conv_id)
    ...
    async def release_conversation_async_impl(self, conv_id: str) -> tuple[bool, str]:
        ...
        if cleanup is not None:
            try:
                await asyncio.to_thread(self._manager._finish_conversation_release, *cleanup)
            finally:
                self._manager._releasing_conversations.discard(f"subagent:{conv_id}")
        return result
