...
class ContextBuilder:
    ...
    def _substitute_bot_name(self, prompt: str, provider_type: str | None = None) -> str:
        ...
        ``provider_type`` only matters when the operator never set a name: the
        default then follows the harness serving the turn, so a claude session
        does not introduce itself as the kiro-cli default. An explicit
        ``agent.bot_name`` -- live or boot-time -- always wins over it.
        """
        snap = live.snapshot()
        live_name = (
            getattr(getattr(snap, "agent", None), "bot_name", "") if snap is not None else ""
        )
        bot_name = live_name or self._bot_name
        if provider_type is not None and not live_name and not self._bot_name_explicit:
    ...
    def build_session_context(
        ...
        *provider_type* is consumed again for the steering gate only: the
        steering block below is injected on every public-spec adapter
        (``is_spec_adapter_provider_label(provider_type)``). kiro-cli loads an agent's
    ...
    def build_message(
        ...
        *,
        ...
        if is_new_session:
            ...
            # ORDER MATTERS. ``slim_resume`` leads: a resumed session already
            # carries whichever prompt was injected at session start, so
            # re-reading one duplicates it. Then a custom agent, which is the
            # user's explicit choice and outranks the branded default persona
            # below -- testing ``is_cc`` before it discards that prompt on every
            # spec adapter and substitutes a persona the user did not pick.
            if slim_resume:
                agent_prompt = ""
            elif is_custom:
...
def _load_agent_file_resources(agent: str, project: str | None = None) -> str:
    ...
    Only loads ``file://`` resources matching ``*.md``.
    """
    # Function-local like the sibling loader above: upstream keeps this reader out
    # of the module's import graph, and this fork-only spec-adapter path is the
    # remaining caller.
    from kiro_crew.agent_discovery import _read_agent_spec
