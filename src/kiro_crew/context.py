...
def _skills_injection_plan(
    agent: str | None, *, is_spec_adapter: bool, project_dir: str | Path | None = None
) -> tuple[bool, list[str]]:
    ...
    # FORK GATE (restored at the 2026-09-21 sync). A custom agent WITH globs is
    # injected only on a SPEC ADAPTER: kiro-cli reads those globs out of the
    # agent's own JSON natively, so injecting there hands it the catalog twice.
    # The parameter is named for the DIALECT, not the vendor (harness-parity
    # H5/H6) -- ``is_cc`` invited the next reader to treat a dialect question as
    # claude-specific, which is how codex silently lost its skills. Upstream's
    # ``bool(globs) or not is_custom`` drops the distinction and injects on both.
...
class ContextBuilder:
    ...
    def __init__(
        ...
        if bot_name:
            ...
            cfg = KiroCrewConfig.load()
            # Reads the ACP BACKEND, not ``agent.provider`` (restored at the
            # 2026-09-21 sync). ``agent.provider`` is enum=["acp"], so asking
            # ``is_claude_code`` about it can only ever answer False and this
            # branding would never fire for anyone. The persona rewrite is
            # Claude-specific, not spec-adapter-wide, so it compares the one
            # backend id rather than widening to every spec dialect.
            #
    ...
    def build_session_context(
        ...
        *provider_type* is consumed again for the steering gate only: the
        ...
        (``provider_type == PROVIDER_LABEL_CLAUDE``). kiro-cli loads an agent's
        ...
        *context_groups* selects which switchable groups are injected (see
        ...
        inject_skills, skill_globs = _skills_injection_plan(
            agent, is_spec_adapter=is_spec_adapter, project_dir=project
        )
        if inject_skills:
    ...
    def build_message(
        ...
        *,
        ...
        # TWO flags, deliberately not one (fork divergence, restored 2026-09-21).
        # Branding compares the EXACT session-map label: rewriting kiro-cli's
        # persona to Claude's wording on a codex session tells the user they are
        # talking to the wrong product. Skills widen to every spec DIALECT,
        # because none of those harnesses loads the agent's globs itself.
        # Collapsing them into one `is_cc` either leaks claude branding onto
        # codex or silently drops codex's skills.
        is_cc = provider_type == PROVIDER_LABEL_CLAUDE
        is_spec_adapter = is_spec_adapter_provider_label(provider_type or "")
        ...
        if not is_new_session and needs_reinjection:
            ...
            _inject, _globs = _skills_injection_plan(
                agent, is_spec_adapter=is_spec_adapter, project_dir=project
            )
            if _inject:
                _cfg = KiroCrewConfig.load()
