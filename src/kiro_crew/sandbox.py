...
_CREW_HIDDEN_LEAVES: tuple[str, ...] = (
    ...
    *_MD_NOTEBOOK_STATE_LEAVES,
    ...
    "token_signing.key",
    "refresh_chains.json",
    # The BYOK store's values are live third-party API keys. It is read only
    # gateway-side, at spawn, to inject the key into the child's environment
    # (``acp/byok.py``), so masking it inside costs no consumer while closing
    # the OS ``open()`` that a spawned ``python -c`` makes without ever routing
    # through ``is_sensitive_path``.
    "byok.json",
    # Gateway-owned process ledgers for orphan recovery. The tool gate already
    # fences them so the agent cannot rewrite its own PID bookkeeping; mask them
    # too, because the in-sandbox readers use the per-session
    # ``session_pid_<pid>.txt`` files instead and never these. The ``.lock``
    # sidecars go with them: a writable lock defeats the ledger it guards.
    "kiro_pids.txt",
    "kiro_pids.lock",
...
_CREW_CHILD_WITHHELD_LEAVES: tuple[str, ...] = (
    ...
    "file_delivery_consent.json",
    "ssh_auth_sock_consent.json",
    # FORK LEAF, classified at the 2026-09-21 sync. It is READONLY here (see the
    # entry in ``_CREW_READONLY_LEAVES``) and upstream carries no such leaf, so
    # upstream's copy of this family had no row to lose -- it is the new
    # completeness pin meeting a leaf only this build has, and the pin's whole
    # point is that the author picks a side rather than defaulting to readable.
    #
    # WITHHELD, on the same two counts as the family above. It is an INPUT TO AN
    # AUTHORIZATION DECISION an in-sandbox process makes: ``security``'s
    # ``_git_publication_floor_applies`` resolves the git-publication floor out of
    # it, and ``workflow_policy`` also decides from it whether the ssh-agent socket
    # is scrubbed. And withholding costs it CLOSED, which is what makes this the
    # safe side: that predicate documents itself as failing to ``True`` -- the
    # floor APPLIES -- on ANY error, so a foreign harness's child that cannot read
    # the file gets the RESTRICTIVE answer, never permission.
    #
    # Note the direction here is the opposite of the READONLY reasoning beside the
    # other entry, and both are right because they answer different readers. For
    # Crew's OWN in-sandbox reader, masking would re-impose a floor the operator
    # deliberately lifted, so it stays readable there. For an ENFORCED harness's
    # foreign child -- which reaches the file through a channel that hits no gate
    # and leaves no record -- the loosening opt-ins are exactly what must not be
    # legible, and losing them only re-imposes a floor.
    "workflow_policy.json",
...
def prime_voice_runtime_sandbox_paths() -> str:
    ...
    with _voice_runtime_paths_lock:
        ...
        # First run: the data home may not exist yet, and an agent spawn must not
        # be the thing that discovers that. Created before it is resolved so the
        # realpath below has something to resolve — without this, a fresh install
        # raises FileNotFoundError here on its first spawn. The lstat check that
        # follows is unchanged, so a home that is not a real directory is still
        # refused; ``config_dir()`` may legitimately BE a symlink, which is why
        # the check runs on the canonical path rather than the lexical one.
        os.makedirs(lexical_home, mode=0o700, exist_ok=True)
...
def _probe_unshare_via_spawn() -> tuple[bool, bool, str, str] | None:
    ...
    try:
        ...
        proc = subprocess.Popen(
            [sys.executable, "-I", "-S", "-B", "-c", _PROBE_SHIM_CODE],
...
#   -B writes no bytecode at all. This is the ONE canonical statement of why -B
#      still exists after the desktop bundle handed bytecode policy to
#      ``PYTHONPYCACHEPREFIX`` (``gateway-env.js``'s
#      ``gatewayBytecodeEnvironment``, which redirects the cache OUTSIDE the
#      signed ``.app`` so a warm start still gets a cache): the redirect is an
#      environment variable, and -I implies -E, so an isolated child ignores
#      every ``PYTHON*`` var including the prefix. A child spawned with
#      ``strip_python_env=True`` never sees it either. Those children would
#      therefore fall back to CPython's DEFAULT location -- beside the bundled
#      sources inside a code-signed bundle -- which is exactly the exposure the
#      prefix closes everywhere it reaches. -B is scoped to the spawns it cannot
#      reach; every one of them is a short-lived one-shot probe or helper that
#      gains nothing from a bytecode cache. Do NOT add -B to a spawn that DOES
#      inherit the prefix: there it silently defeats the warm start.
...
_LAUNCHER_INTERPRETER_FLAGS: tuple[str, ...] = ("-I", "-S", "-B")
...
def _refuse_macos_sandbox_owner_conflict(argv: list[str], detail: str) -> NoReturn:
    """Refuse a Kiro spawn whose required path policy cannot be delegated.

    Kiro's internal Seatbelt layer and Kiro Crew's ``sandbox-exec`` layer cannot
    nest. A path deny that only Kiro Crew understands therefore has no safe
    execution shape while the internal layer is enabled: delegating drops the
    deny, while wrapping prevents the child from starting.
    """
    try:
        from kiro_crew.sel import sel  # circular import: sandbox is low-level
...
def wrap_argv(
    ...
    *,
    ...
    if darwin_kiro_internal_sandbox and _run_coordinator_uses_custom_home():
        _refuse_macos_sandbox_owner_conflict(
            argv,
    ...
    # Windows never reads the macOS internal-sandbox setting: a positively
    # identified Kiro spawn delegates to Kiro's built-in sandbox unconditionally.
    delegate_to_kiro = darwin_kiro_internal_sandbox or (
        sys.platform == "win32" and is_kiro_cli is True
    )
    if private_memory:
        # Private member memory needs Crew's OWN boundary: a delegated
        # sandbox is the harness's, and this core cannot verify what it
        # hides. Refusing to delegate keeps the fail-closed direction.
...
def spawn_shim_argv(
    ...
    *ctty_fd* asks the shim to make the terminal on that inherited descriptor the
    ...
    argv = [sys.executable, "-I", "-S", "-B", "-c", _SPAWN_SHIM_CODE]
    if spec:
