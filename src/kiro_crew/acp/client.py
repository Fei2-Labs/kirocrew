...
def _resolve_opencode_bin() -> str | None:
    """Find the OpenCode executable for the ``opencode`` ACP backend.

    Resolution order:
      1. ``OPENCODE_ACP_BIN`` env var (explicit override).
      2. Augmented PATH (covers Homebrew, `npm i -g opencode-ai`, the
         install-script layout `~/.opencode/bin`, mise/asdf/nvm shims).
    """
    override = os.environ.get("OPENCODE_ACP_BIN")
    if override and Path(override).is_file():
        return override
...
def _wire_contract_for_backend(backend: str) -> tuple[int | str, dict[str, Any]]:
    ...
    # one-line edit here rather than a silent downgrade of whichever harness moved
    # first. The dialect supplies the default; a backend that has stated its own
    # version overrides it. Both currently equal PROTOCOL_VERSION_SPEC, and that
    # is the point -- they are three literals that happen to agree today.
    per_backend_version: dict[str, int | str] = {
    ...
    return per_backend_version.get(backend, version), capabilities
...
class AcpClient:
    ...
    def _mirror_session_mcp_servers(self) -> list[dict[str, Any]]:
    ...
    def _claude_session_mcp_servers(self) -> list:
        ...
        name and does the work. The mirror translation upstream added is reached
        through :meth:`_mirror_session_mcp_servers`, which the array owner
        :meth:`_session_mcp_servers` folds in -- it is composed with the spec
        pipeline, not a second delivery path for the same servers.
        """
    ...
    async def _session_mcp_servers(self) -> list:
        ...
        # ``ACP_BACKENDS_SESSION_MCP_ARRAY`` the mirror translation of the agent
        # spec (upstream's ``_mirror_session_mcp_servers``) is the source of the
        # user-facing entries, while ``managed_spec_servers`` supplies Crew's own
        # control-plane servers. Both belong on the array, and the merge is what
        # keeps a name declared by both from landing twice.
        mirrored = await asyncio.to_thread(self._mirror_session_mcp_servers)
        # Managed second so Crew's own control-plane entry wins a name clash
        # with the mirror's translation of the same server.
        managed = spec_servers.merge_session_servers(mirrored, managed)
    ...
    async def _spawn(self) -> None:
        ...
        if self._is_claude:
            ...
            # message that reports it. Resolving against the live ``os.environ``
            # and then recomputing the directory set from a FRESH read on failure
            # lets a PATH change landing in that window (a concurrent installer,
            # a self-update, anything editing the gateway's environment) produce
            # a "not found" message naming directories that were never searched
            # while omitting ones that were. Same guarantee the Claude adapter
            # gets by caching its search path with the resolution result.
