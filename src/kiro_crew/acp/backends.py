...
_COPILOT = BackendDescriptor(
    # Fork-only backend: GitHub Copilot CLI's own ACP server mode
    # (`copilot --acp`), verified against a live process — it answers
    # `initialize` with protocolVersion 1 and a normal ACP payload
    # (agentInfo.name == "Copilot"). One process per session on the AcpClient
    # path, like the other spec adapters.
    id=ACP_BACKEND_COPILOT,
    label="GitHub Copilot",
    experimental=True,
    dialect=Dialect.SPEC,
    routing=Routing.PERMISSION_REQUEST,
    signin_command="copilot auth login",
    # A single first-party binary; the GitHub Copilot CLI installer/updater
    # owns distribution, so there is no adapter package to name here.
    install_command="",
    registry_id="",
    # Copilot's OAuth token lives in the gh/copilot config; Kiro Crew never
    # reads it, so no credential leaf is named.
    credential_leaves=(),
    process_markers=("copilot",),
    permission_config_id="",
    permission_config_value="",
    capabilities={
        CAP_SESSION_SHARING: Level.UNAVAILABLE,
        ...
        CAP_TOOL_SEARCH: Level.UNAVAILABLE,
        # Prompt instructions and skills are injected; restricted tool
        # profiles cannot be enforced.
        CAP_AGENT_PROFILES: Level.DEGRADED,
        CAP_SLASH_COMMANDS: Level.DEGRADED,
        CAP_TURN_USAGE: Level.UNVERIFIED,
        CAP_BILLING: Level.UNAVAILABLE,
        # Verified on a live `copilot --acp` process: it advertises
        # loadSession=true and session/load keyed by sessionId restores the
        # session (fork-driven end to end before the descriptor layer landed).
        CAP_NATIVE_RESUME: Level.SUPPORTED,
        # Copilot ids (`gpt-*`, `claude-*` as served by Copilot) are not
        # model_registry keys.
        CAP_REGISTRY_MODEL_IDS: Level.UNAVAILABLE,
        CAP_MID_TURN_STEER: Level.UNAVAILABLE,
    },
...
_OPENCODE = BackendDescriptor(
    ...
    capabilities={
        ...
        CAP_TURN_USAGE: Level.UNVERIFIED,
        CAP_BILLING: Level.UNAVAILABLE,
        # Verified on a live `opencode acp` process (v1.14.18): it advertises
        # loadSession=true and session/load keyed by sessionId restores the
        # session (fork-driven end to end before the descriptor layer landed).
        CAP_NATIVE_RESUME: Level.SUPPORTED,
        CAP_REGISTRY_MODEL_IDS: Level.UNAVAILABLE,
