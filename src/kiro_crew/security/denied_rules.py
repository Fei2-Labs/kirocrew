...
BUILTIN_DENIED_RULES: list[DeniedCommandRule] = [
    ...
    DeniedCommandRule(
        ...
        pattern="(?:\\A|[;&|\\n`]|\\$\\()[\\s\"'(]*(?:[\\w.:+/\\\\-]*[/\\\\])?kiro[-.]?crew(?:\\.\\w+)?\\b[\"']*(?:[^\\S\\n][^;&|#>/*\\n]*)?[^\\S\\n]\\$?[\"']?token\\b",
        category="credential-exfil",
        description="Blocks the `kirocrew token` CLI, which mints a signed dashboard access token an attacker could use to authenticate to the gateway. Matches the CLI name in command position -- optionally path-qualified, including the hyphenated `kiro-crew` spelling -- followed on the SAME LINE by the token verb, with a gap that admits anything except a command separator, comment, redirect, glob, or path character, covering forms such as `kirocrew --port 6777 pod token`. An identifier that merely contains the name (a rule id, or a product-named file path) is not a mint, and a mention on a different line of a multi-line text no longer matches. The argv floor additionally covers `python -m kiro_crew ... token`, which mints the same token through the interpreter rather than the console script.",
    ),
    DeniedCommandRule(
...
def floor_enforced_builtin_command_ids() -> frozenset[str]:
    ...
    is the verb-anchored git-publish floor (``_is_git_publish`` /
    ``_is_push_to_protected_branch``) evaluated before the configurable tiers,
    which consults no PER-RULE opt-out state.  It has exactly one switch, and it
    is not a rule id: the keystone ``workflow_policy.json`` can place the whole
    floor in repository-governed mode (``_git_publication_floor_applies``), which
    is an operator decision on a file the agent can neither read nor write.
    Persisting one of these ids into
    ...
    cannot weaken the floor.  Safe to call from any thread.

    Empty in repository-governed mode, and that is the point: with the floor stood
    down there is nothing forced on, so rendering these rows as locked would tell
    the operator the opposite of what the gate will do.  Read through the same
    predicate the enforcement path uses, so the two cannot disagree about the mode.
    """
    from kiro_crew.security import _git_publication_floor_applies
