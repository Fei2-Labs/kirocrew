...
class SessionLifecycleService:
    ...
    async def refresh_defaults(self) -> None:
        ...
        # Off the loop: config validation touches disk, so a settings save must
        # not stall every other request being served while it lands.
    ...
    async def reset(
        ...
        *,
        ...
        if session:
            ...
            # The fourth helper is the PID-reuse guard, which the subagent
            # reaper resolves through this same seam; reset does not need it
            # because the records it captures already carry start times.
