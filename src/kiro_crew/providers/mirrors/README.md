...
| `copilot` | `NO_MIRROR` | selectable; `PERMISSION_REQUEST` routing and no native config write, so the `session/new` array is the whole surface |
| `opencode` | `NO_MIRROR`, pending | selectable; `SEEDED_SETTINGS` projection already exists in `acp/opencode.py`, not yet moved here |
| `goose` | `NO_MIRROR` | not selectable, and `_spec_session_mcp_delivery` withholds Crew's control plane from it by name |
| `pi` | `NO_MIRROR` | known but not selectable; `PERMISSION_REQUEST`, so it would inherit copilot's shape |
