...
class TestServedBackendAttribution:
    ...
    def test_label_distinguishes_the_backend_the_config_cannot(self):
        """`provider_label` separates every backend this fork can serve.

        `is_claude_backend` is the narrower question -- it answers "claude or
        not", which collapses KAS, copilot, goose, opencode and pi onto one
        label. This fork drives all of them, so the emit site resolves through
        `provider_label` instead and this pins the vocabulary it returns.
        """
        ...
        # A KAS turn is labelled "kas", not "acp": the distinction the narrower
        # boolean cannot draw is exactly why the emit site reads this resolver.
    ...
    def test_chat_runner_resolves_the_label_through_a_ratcheted_edge(self):
        ...
        `scripts/check_agent_sdk_boundary.py` refuses an ACP-layer import in a
        file that is NOT in `.github/agent-sdk-boundary-baseline.txt`, and
        refuses a baselined file's count growing. chat_runner's
        `providers.acp` edge is pre-existing and recorded there, so reading
        `provider_label` is governed by the shrink-only ratchet rather than
        forbidden -- which is why the emit site can name every served backend
        instead of collapsing them onto "claude_code or acp".

        Both halves are pinned: the symbol is genuinely imported (checked as an
        IMPORTED NAME via AST, not as a substring, since the emit site's own
        comment names it in prose), and the file still carries a baseline entry.
        Losing the entry is what would turn this edge into a gate failure.
        """
        import ast
        from pathlib import Path
        ...
        rel = "src/kiro_crew/dashboard/chat_runner.py"
        recorded = [
            line for line in baseline.read_text(encoding="utf-8").splitlines() if line.endswith(rel)
        ]
        assert recorded, (
            f"{rel} is no longer in the agent-sdk boundary baseline, so its "
            "provider_label import is now an ungoverned violation. Either restore "
            "the entry or move the label onto kiro_crew.agent_sdk."
        )
