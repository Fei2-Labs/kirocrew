...
async def test_client_session_requests_preserve_ordinary_broker_routing(
    ...
    # This test asserts on the pre-seeded `_session_mcp_cache` reaching the wire,
    # not on the tool-gate's own routing verdict -- stub that ROUTED so the gate
    # this build applies uniformly to every non-kiro backend (including claude)
    # does not withhold delivery for a session with no real settings.local.json.
    if backend == ACP_BACKEND_CLAUDE:
        from kiro_crew.acp import tool_gate
        from kiro_crew.acp.codex import Verdict

        monkeypatch.setattr(tool_gate, "resolve_verdict", lambda *a, **k: (Verdict.ROUTED, "test"))
