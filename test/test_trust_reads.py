...
class TestIsReadOnlyBash:
    ...
    def test_elision_refusal_is_scoped_to_verbs_a_phantom_word_can_flip(self):
        """Flag-decided reads keep comments and input redirects auto-approved.

        The later per-verb elision gate derives its scope from positional and
        mode-sensitive tables. A deleted word can flip those verdicts, but for a
        flag-decided verb it can only add an inert word or an extra prompt.
        """
        for cmd in (
            "git status # note",
            "ls -la # list files",
