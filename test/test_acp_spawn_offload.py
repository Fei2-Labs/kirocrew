...
class TestRuntimeShieldSurvivesAFailedAppend:
    ...
    async def test_failed_spawn_unwinds_live_tree_and_tracking(
        ...
        # The FORK keeps its pid files under a ``run/`` subdirectory of the data
        # home (``session_pid._pid_file_path``); the merge base and upstream put
        # them flat, which is the layout this upstream-authored test assumed.
        # Build the subdir so the writes land where the code actually looks --
        # flattening the code instead would move an operator's real pid files.
        pid_dir = tmp_path / "run"
        pid_dir.mkdir(parents=True, exist_ok=True)
        paths = [pid_dir / "kiro_pids.txt", pid_dir / "kiro_session_pids.txt"]
