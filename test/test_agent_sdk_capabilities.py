...
def test_known_membership_is_unchanged_by_the_move() -> None:
    ...
    assert sorted(sdk_backends.ACP_BACKENDS_KNOWN) == [
        ...
        "claude",
        "codex",
        # FORK-ONLY, and additive: GitHub Copilot CLI's own ``--acp`` server, whose
        # tool calls route through Crew's PreToolUse gate. Upstream's own list is
        # the rest of this literal; see .fork-sync.yml divergences/copilot-selectable.
        "copilot",
        "deepseek",
        "goose",
        "kas",
