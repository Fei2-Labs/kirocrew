...
def test_the_unverified_known_backends_are_deepseek_and_copilot() -> None:
    """The audit the refusal rests on, kept as a test so it cannot go quietly stale.

    If a further harness ever resolves to ``UNVERIFIED`` -- including by being
    absent from the routing table, which ``routing_for`` answers ``UNVERIFIED``
    for -- the refusal above starts applying to it too. That may be right, but it
    must be noticed rather than discovered when an edition's registration begins
    failing.

    WIDENED at the 2026-09-21 sync, and the added id is the reason the last
    assertion had to change shape rather than just gain a member.

    ``copilot`` is FORK-ONLY: it is in this build's
    ``BASELINE_SELECTABLE_BACKENDS`` and not in upstream's, so upstream's audit
    could honestly say deepseek was the only one. Here it is both SELECTABLE and
    absent from ``ACP_BACKEND_ROUTING``, which is pre-existing fork state and not
    something this merge introduced -- the pre-merge fork resolves the same pair
    -- and ``BASELINE_SELECTABLE_BACKENDS``' own docstring names it as "the one
    place this set is looser than the criterion above".

    That looseness is bounded, and this is what bounds it: copilot's tool calls
    ARE gated, through the OTHER routing declaration. ``acp.backends``' descriptor
    for it carries ``Routing.PERMISSION_REQUEST`` -- a member the ``agent_sdk``
    enum does not have -- and ``acp.tool_gate.resolve_verdict("copilot", ...)``
    answers ``Verdict.ROUTED``. So the ``UNVERIFIED`` here is the absence of a row
    in the sdk-side table, not a session reaching tools ungated, and the assertion
    below pins the gated half so this stays a bookkeeping gap rather than
    silently becoming a real one.

    Do NOT close this by inventing an ``ACP_BACKEND_ROUTING`` row for copilot:
    that table's members each name a mechanism Crew performs and reads back, and
    copilot's mechanism (the adapter asking per call) has no member there. The
    honest fix is upstream adding the member, or the descriptor table becoming the
    single source -- either way a change to the code, not to this pin.
    """
    unverified = {b for b in ACP_BACKENDS_KNOWN if routing_for(b) is Routing.UNVERIFIED}
    assert unverified == {ACP_BACKEND_DEEPSEEK, ACP_BACKEND_COPILOT}
    # Every known id EXCEPT the fork's copilot is named explicitly, so none of the
    # others is unverified merely by omission.
    from kiro_crew.acp_backends import ACP_BACKEND_ROUTING

    assert set(ACP_BACKEND_ROUTING) >= ACP_BACKENDS_KNOWN - {ACP_BACKEND_COPILOT}
    # deepseek stays out of the shipped baseline, which is what makes the refusal a
    # no-op for it. copilot is IN the baseline, so its guarantee has to come from
    # the gate itself rather than from non-selectability -- asserted, not assumed.
    assert not (unverified - {ACP_BACKEND_COPILOT}) & set(BASELINE_SELECTABLE_BACKENDS)
    from kiro_crew.acp.tool_gate import Verdict, resolve_verdict

    verdict, _reason = resolve_verdict(ACP_BACKEND_COPILOT, pathlib.Path("/tmp"))
