...
# ── The install snapshot the endpoint merges in ──


class TestInstallSnapshot:
    """The pinned machine half of ``GET /api/acp-backends``'s rows.

    This module owns the snapshot only. The endpoint, its owner gate and the
    merge with the registry descriptors live in
    ``kiro_crew.dashboard.handlers.acp_backends`` and are covered by
    ``test_acp_backends_endpoint.py`` -- one handler on one route, so there is
    one place each property is asserted.
    """

    def test_one_entry_per_known_backend_in_the_pinned_shape(self, monkeypatch):
        ...
        # Derived, not hardcoded: this fork's ``ACP_BACKENDS_KNOWN`` carries the
        # operator-installable adapters too, so a literal list here would pin
    ...
    def test_an_unknown_row_names_no_components(self, monkeypatch):
        ...
        snapshot = install_snapshot()
        assert snapshot[ACP_BACKEND_KIRO]["installed"] == "unknown"
        assert snapshot[ACP_BACKEND_KIRO]["missing_components"] == []
        assert snapshot[ACP_BACKEND_KAS]["installed"] == "unknown"
