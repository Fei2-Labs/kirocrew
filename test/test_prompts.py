...
class TestCreateAndDeletePinTheDirectory:
    ...
    def test_the_named_fallback_still_refuses_an_occupied_name(
        ...
        d = tmp_path / ".kiro" / "prompts"
        resident = d / "byname.md"
        before = resident.stat()
        ...
        assert resp.status == 409
        assert json.loads(resp.body)["code"] == "prompt_exists"
        assert resident.read_text() == "RESIDENT\n"
        # Same INODE, not merely the same bytes: `O_EXCL` must refuse the name
        # outright rather than write through it and restore the content.
        assert (resident.stat().st_dev, resident.stat().st_ino) == (before.st_dev, before.st_ino)
        # And nothing beside it -- a refused create leaves no temp behind.
        assert [q.name for q in d.iterdir()] == ["byname.md"]
    ...
    def test_the_named_fallback_withdraws_a_body_it_could_not_write(
        self, tmp_path, mock_sel, monkeypatch, force_named_fallback
    ):
        """A create that dies mid-BODY leaves no name and no debris.

        The fallback's substitute for the unnamed path's "the name appears only
        on success". Here `O_EXCL` publishes the name FIRST, so the promise has
        to be kept by the identity-checked cleanup arm instead: it removes the
        leaf this call created, so a retry gets a clean create rather than the
        truncated leftover that answers 409 forever.

        Distinct from `test_the_named_fallback_withdraws_a_prompt_it_cannot_flush`,
        which fails the DIRECTORY flush -- by then the body is already written,
        so it never exercises the arm reached before `created_ident` has a
        ...
        def _fail_the_body(fd, data):
            if data == b"BODY" and not failed["body"]:
                failed["body"] = True
                raise OSError(errno.ENOSPC, "No space left on device")
        ...
        resp = asyncio.run(
            api_prompts_create(_create_request({"name": "halfbody", "content": "BODY"}))
        )
        monkeypatch.setattr(os, "write", real_write)

        assert failed["body"], "the body write never failed — the test would be vacuous"
        assert resp.status == 500
        assert json.loads(resp.body)["code"] == "write_failed"
        assert not (
            d / "halfbody.md"
        ).exists(), "a create that could not write its body kept a name"
        assert (
            list(d.iterdir()) == []
        ), f"debris left beside the withdrawn leaf: {list(d.iterdir())}"
        assert _outcomes(mock_sel)[-1] == "error"

    def test_the_named_fallback_leaves_an_incumbent_untouched_when_a_write_fails(
        self, tmp_path, mock_sel, monkeypatch, force_named_fallback
    ):
        """The two halves together: an occupied name AND a failing write.

        The unnamed-path sibling below asserts this pair on `O_TMPFILE` hosts.
        On the fallback the ordering is the other way round -- `O_EXCL` refuses
        before a byte is written -- so the incumbent is safe for a different
        reason, and that reason is what is pinned here: no write is even
        attempted at a name someone else holds.
        """
        d = tmp_path / ".kiro" / "prompts"
        _user_prompt(tmp_path, "rival", "NOT YOURS")
        rival = d / "rival.md"
        before = rival.stat()
        ...
        monkeypatch.setattr(os, "write", _record)
        taken = asyncio.run(
        ...
        assert not attempted["write"], (
            "the fallback wrote the body before discovering the name was taken; "
            "O_EXCL must refuse the open, not the publish"
        )
        assert rival.read_text() == "NOT YOURS"
        assert (rival.stat().st_dev, rival.stat().st_ino) == (before.st_dev, before.st_ino)
        assert [q.name for q in d.iterdir()] == ["rival.md"]
