...
def test_the_cli_version_flag_reports_the_build(tmp_path) -> None:
    ...
    assert proc.returncode == 0, proc.stderr
    # This build appends its fork identity as a PEP 440 local segment on display
    # surfaces (.fork-sync.yml, divergences/fork-version-identity); the stamp is
    # the public half before it.
    assert proc.stdout.strip().split("+", 1)[0] == f"kirocrew {stamped}"
