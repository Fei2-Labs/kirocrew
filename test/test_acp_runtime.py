...
class TestAcpRuntimeLoadSession:
    ...
    def test_every_session_request_builder_consults_pooled_servers(self):
        ...
        for name, body in builders.items():
            ...
            # every non-kiro builder reaches the pooled stubs through the ONE
            # tool-gate-gated call, which resolves them via _pooled_mcp_servers.
            assert any(
                hook in body
                for hook in (
                    "pooled_session_servers",
                    "_pooled_mcp_servers",
                    "_gated_session_mcp_servers",
                )
            ), (
