...
class TestInjectionNoticeOutcome:
    ...
    def test_failed_run_does_not_claim_finished(self) -> None:
        info = _info(done=True, error="Timed out after 30 minutes")
    ...
    def test_rejection_sentinels_read_as_rejected(self) -> None:
        for err in ("spawn rejected", "spawn refused: only 1.0 GB memory available (need 4 GB)"):
            info = _info(done=True, error=err)
            assert sa._injection_notice_outcome(info) == (
                "The run was rejected before it started, so there is no result to deliver."
            ), err

    def test_rejected_with_output_keeps_the_recovery_hint_honest(self) -> None:
        # Defensive: a record carrying output must never be described as
        # having no result to deliver — the failed copy stays truthful.
...
class TestNotifyInjectionFailedOutcomeCopy:
    ...
    @pytest.mark.asyncio
    async def test_approval_rejection_reads_as_rejected(self) -> None:
        info = _info(parent_session_key="dash:1", done=True, error="spawn rejected")
        msg = await self._notice_for(info)
        assert "The run was rejected before it started" in msg
        assert "finished" not in msg

    @pytest.mark.asyncio
    async def test_queued_rejection_reads_as_rejected_without_mechanism_detail(self) -> None:
        info = _info(
            parent_session_key="dash:1",
            done=True,
