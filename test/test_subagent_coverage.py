...
class TestAnnounceDigestFlush:
    ...
    def test_settle_swallows_tombstone_failure(self) -> None:
        # SYNCHRONOUS at the 2026-09-21 sync. The fork had made this ``async``
        # solely so it could ``await self._manager._ack_delivery_for_run(_hid)``
        # -- the run coordinator's outbox ack. That coordinator is retired in
        # favour of upstream's taskq (rfc-overload-resilience supersedes
        # rfc-durable-run-coordinator), ``_ack_delivery_for_run`` no longer
        # exists, and upstream's settle is a plain sync call. Awaiting it now
        # awaits ``None``; what this test pins -- an unwritable run folder is
        # logged and skipped rather than stranding the rest of the chunk -- is
