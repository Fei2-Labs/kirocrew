...
def test_self_heal_runs_fixed_pip_argv(monkeypatch, tmp_path):
    ...
    def _fake_run(argv, **kwargs):
        if argv[1:3] == ["-m", "pip"]:
            ...
            script.chmod(0o755)
        # This build adds -B to every isolated (-I) spawn: the pycache prefix cannot
        # reach such a child (test_build_desktop_launcher_symlink pins the policy).
        elif argv[1:] == ["-I", "-B", "-X", "utf8", "-c", "import kiro_crew"]:
