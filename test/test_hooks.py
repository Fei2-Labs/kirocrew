...
class TestHookGateKwargs:
    ...
    EVENT_FIELD_BY_KWARG = {
        ...
        # FORK ADDITION: the boundary-ambiguity flag is an enforcement input the
        # fork's gate takes, so the shared extraction owns it too.
        "mcp_identity_ambiguous": "mcp_identity_ambiguous",
    }
    ...
    def test_hook_gate_kwargs_covers_every_event_derived_gate_parameter(self):
        ...
        probe.mcp_identity_ambiguous = True
        got = hook_gate_kwargs(probe)
        for kwarg, attr in self.EVENT_FIELD_BY_KWARG.items():
            if kwarg in ("is_shell", "mcp_identity_trusted", "mcp_identity_ambiguous"):
    ...
    ALLOWED_OVERRIDES = {
        # FORK ADDITION: the worker-pool permission gate reads the identity off
        # the PERMISSION payload, which is untrusted there (the trusted channel
        # rides the tool_call frame, not this one), so it pins the flag off. It
        # still threads every other field through the shared extraction.
        "acp/client.py": {"mcp_identity_trusted"},
