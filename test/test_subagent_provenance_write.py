...
async def test_pid_and_session_records_are_written_off_loop() -> None:
    ...
    # Admitted through the coordinator, because the PID record is a FENCED
    # lifecycle write: `_record_process_identity` persists process identity
    # through the run's execution fence, so a run that never went through
    # durable admission has no fence and aborts before the write under test.
    # `_run` marks the run STARTING before it awaits `_run_inner`, so driving
    # `_run_inner` directly has to do that half itself.
    await coordinator.submit(
        SubmitRun(
            run_id=info.id,
            command_id="offloop1-command",
            idempotency_key="offloop1-key",
            payload_hash="offloop1-hash",
            parent_session="",
            agent="",
            task=info.task,
            conversation_key="",
