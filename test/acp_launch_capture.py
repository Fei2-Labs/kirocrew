...
#: FORK BACKEND. Stubbed for the same reason as every other bin here: this one
#: resolves through ``_resolve_copilot_bin`` rather than the self-served table
#: below, so without a stub the capture walks the RECORDING HOST's PATH and
#: records whatever real ``copilot`` it finds (``/opt/homebrew/bin/copilot`` on
#: the machine this row was first generated on). That makes the fixture a
#: property of the host instead of the code, which is the one thing
#: ``test_no_golden_entry_carries_a_host_path`` exists to catch.
_COPILOT_BIN = "/opt/bin/copilot"
...
def _stub_common(stack: list, rec: _Recorder, tmp_path: Path) -> None:
    ...
    stack.extend(
        [
            ...
            # The fork's copilot backend does not go through the table above, so it
            # needs its own stub -- see ``_COPILOT_BIN``.
            patch.object(client_mod, "_resolve_copilot_bin", return_value=_COPILOT_BIN),
        ]
    )
