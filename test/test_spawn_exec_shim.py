...
class TestSpawnShimArgv:
    def test_prefix_is_an_isolated_interpreter_running_captured_source(self):
        ...
        # -I keeps env/user-site out; -S additionally skips site. -B is required
        # here because -I implies -E: this child ignores PYTHONPYCACHEPREFIX, the
        # single owner of bytecode policy for every child the redirect DOES reach
        # (website/electron/gateway-env.js gatewayBytecodeEnvironment). See
        # .fork-sync.yml signed-bundle-bytecode-floor, scoped rather than retired.
        assert prefix[1:5] == ("-I", "-S", "-B", "-c")
