...
> **Fei2-Labs fork.** This repository tracks
> [kirodotdev/KiroCrew](https://github.com/kirodotdev/KiroCrew) and adds
> operator-installed ACP backends selected through `agent.acp_backend`.
> GitHub Copilot runs through `copilot --acp`; OpenCode supplies the BYOK path
> through `opencode acp`. Both use Kiro Crew's existing session, sandbox, MCP,
> and tool-permission seams. An adapter whose tool calls cannot reach Kiro
> Crew's gate is refused by default. `kiro-cli` remains the first-class and
> default backend; `agent.provider` remains `acp`.
...
always-on servers, or a build from source. The default setup runs on `kiro-cli`,
so the first launch installs it if needed and guides Kiro device-code sign-in.
Operator-installed ACP adapters can instead be selected with
`agent.acp_backend`.
