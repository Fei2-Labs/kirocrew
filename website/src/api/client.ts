...
export interface AcpBackendProbe {
  ...
  /** Rich descriptor fields returned when the endpoint is probed. */
  label?: string
...
/** Combined response shape: unprobed requests expose registry/probe rows, while
 * probed requests add active routing metadata and richer row descriptors. */
export interface AcpBackendsResponse {
...
export const api = {
  ...
  acpBackends: ({ probe }: { probe?: boolean } = {}) =>
    fetch('/api/acp-backends' + (probe ? '?probe=1' : '')).then(j) as Promise<AcpBackendsResponse>,
  ...
  createChatSlot: (name?: string, agent?: string, model?: string, mode?: string, memory_mode?: string, title?: string, clean_mode?: boolean, artifact?: string, folder_id?: string, project?: string | null) => post('/api/chat/slots', { ...(name ? { name } : {}), ...(agent ? { agent } : {}), ...(model ? { model } : {}), ...(mode ? { mode } : {}), ...(memory_mode ? { memory_mode } : {}), ...(title ? { title } : {}), ...(clean_mode !== undefined ? { clean_mode } : {}), ...(artifact ? { artifact } : {}), ...(folder_id ? { folder_id } : {}), ...(project !== undefined ? { project } : {}) }).then(j),
