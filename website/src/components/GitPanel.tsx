...
import { FolderGit2, GitBranch, RefreshCw, RefreshCwOff } from 'lucide-react'
...
export default function GitPanel({ projectDir, onFileOpen, onClose }: GitPanelProps) {
  ...
  // The discovery scan is cached for 60s server-side; the refresh BUTTON must
  // see a repo cloned into the workspace rather than wait out that TTL, so it
  // arms a one-shot `?refresh=1` for the next fetch (fork feature 5cb712fb5).
  ...
  // FORK FEATURE (5cb712fb5, not upstream's): a project dir that is not itself a
  // repo answers with a `repos` array, one entry per discovered checkout. A
  // single-repo answer collapses to ONE unnamed group, so the ordinary case
  ...
  return (
          ...
          {statusError ? (
            ...
            <>
              ...
              <span className="text-[12px] font-medium text-text truncate">
                {status?.branch
                  // A multi-repo answer has no single branch to name, so the
                  // header states how many checkouts were discovered instead of
                  // falling through to "not a repository" (fork feature
                  // 5cb712fb5) -- the project dir genuinely is not one, but it
                  // HOLDS several, which is the fact the reader needs.
                  || (status?.repos?.length
    ...
    >
      <div className="overflow-y-auto flex-1 text-[12px]">
        ...
        {/* ── CHANGES section ── */}
        {/* FORK FEATURE (5cb712fb5): a workspace whose only signal is a SKIPPED
            or capped repo still has something to say, so the section renders on
            those too -- otherwise the refusal is silent and the panel looks
            clean for a repo it never read. */}
        {!statusError
          && isRepository
          && (!hasNoChanges
            || status?.reposTruncated
            || status?.refused
            || groups.some(g => g.refused || g.truncated)) && (
          <section className="py-2">
            ...
            <div>
              {groups.map(group => (
                <div key={group.root}>
                  {/* A named group means several repos were discovered; the
                      single-repo group carries no name and renders no header. */}
                  {group.name && (
                        ...
                        : group.truncated
                          ? (
                            <span
                              ...
                              title={i18nT('components.gitPanel.showing_first', { count: fileCount })}
                            >
                              &mdash;
                            </span>
                          )
