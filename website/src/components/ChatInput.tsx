...
import { Badge, Btn, Slider } from './ui'
...
/** Human label for a persisted ACP backend id. Falls through to the raw id so a
 *  backend the dashboard does not yet know still names itself rather than
 *  rendering blank. */
...
interface ChatInputProps {
  ...
  canSteer?: boolean
  /** Live ACP provider label, used for backend-specific composer affordances. */
...
function ChatInput({
  ...
  return (
    ...
    <div className={`input-area px-4 pb-1 ${hasApproval ? 'pt-0' : 'pt-1'} mx-auto w-full flex flex-col`}
      ...
      {!showGhost &&
        !composerCollapsed &&
        // The provider badge counts as a reason to draw the shelf, for the same
        // reason the app chip below does: without it the badge is declared,
        // mounted and unreachable whenever no other pill happens to be present.
        (providerLabel ||
          onProjectClick ||
