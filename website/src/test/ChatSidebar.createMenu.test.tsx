  ...
  it('lists each connected crew and creates the session on the one picked', async () => {
    ...
    await waitFor(() =>
      expect(mocks.createChatSlot).toHaveBeenCalledWith(
        ...
        undefined,
        // This build's own create tail, after upstream's `agent_kind`:
        // `clean_mode`, then `project`. Both undefined HERE for their own
        // reason, not as padding — the crew row names neither, and `project`
        // falls through to the active slot's, which this test has none of.
        undefined,
        undefined,
      ),
    )
