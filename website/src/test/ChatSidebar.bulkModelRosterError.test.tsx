...
beforeEach(() => {
  localStorage.clear()
  // Cache rows are namespace-bound. The bare-array API shape is Kiro, whose
  // canonical backend id is the empty string; seed that last-known namespace
  // so cache fixtures model a real previously successful fetch.
  localStorage.setItem('kc.acp.backend.v1', '')
...
describe('ChatSidebar — Switch All Sessions roster failure', () => {
  ...
  it('shows the notice when a failed fetch serves the last-good cached list', async () => {
    ...
    await openSwitchAllPanel()
    expect(await screen.findByTestId('bulk-model-roster-error')).toBeTruthy()
    await waitFor(() => expect(optionIds()).toEqual(['auto', 'opus-4.8']))
  })
