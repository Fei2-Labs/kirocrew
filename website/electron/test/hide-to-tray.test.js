...
describe("window lifecycle tray-close wiring", () => {
  ...
  it("keeps the fork's gateway controls on the tray menu", () => {
    // A fork divergence that lives entirely in the tray template. Upstream
    // extracted the tray out of main.js, so the items must follow it here --
    // pinned because a re-extraction would otherwise drop them without a
    // conflict marker, leaving the label and the actions silently gone.
    assert.match(WINDOW_LIFECYCLE_JS, /label: `Gateway: \$\{status\}`/);
    for (const action of ["Start", "Stop", "Restart"]) {
      assert.match(
        WINDOW_LIFECYCLE_JS,
        new RegExp(`label: "${action} Gateway"[\\s\\S]*?gatewayControls\\.run\\("${action.toLowerCase()}"\\)`),
        `the tray must still offer ${action} Gateway`,
      );
    }
    // The label is only honest if it is rebuilt, not built once at boot.
    assert.match(WINDOW_LIFECYCLE_JS, /refreshTrayMenu = \(\) => \{/);
    assert.match(MAIN_JS, /gateway\.observeState\(\(\) => windows\.refreshTrayMenu\(\)\)/);
  });
