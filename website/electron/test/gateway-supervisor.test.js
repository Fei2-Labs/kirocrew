...
// ── Tray gateway controls ──
// A fork divergence: the tray offers Start/Stop/Restart beside a live status
// label. It is UNPINNED nowhere else, and an upstream sync that re-extracts
// main.js would otherwise drop it silently, so the seam is pinned here.

test("controlState: a fresh supervisor reports a stopped, unowned gateway", () => {
  const { supervisor } = harness();
  assert.deepStrictEqual(supervisor.controlState(), {
    running: false,
    owned: false,
    operation: null,
  });
...
test("controlState: a spawned child reads as running and owned", async () => {
  const { supervisor } = harness();
...
test("observeState: every spawn transition notifies the tray", async () => {
  const { supervisor } = harness();
  let notifications = 0;
  supervisor.observeState(() => { notifications += 1; });
  await supervisor.start();
  assert.ok(notifications > 0, "the spawn must redraw the tray label");
});

test("observeState: a throwing observer cannot break a gateway start", async () => {
  const { supervisor, logs } = harness();
  supervisor.observeState(() => { throw new Error("observer boom"); });
  await supervisor.start();
  assert.deepStrictEqual(supervisor.controlState().owned, true);
  assert.ok(
    logs.some((line) => line.includes("gateway state observer threw")),
    "the swallowed observer failure must still be logged",
  );
});

test("runControlAction: stop and restart are refused while nothing is owned", async () => {
  const { supervisor, spawnCalls } = harness();
  await supervisor.runControlAction("stop");
  await supervisor.runControlAction("restart");
  assert.deepStrictEqual(spawnCalls.length, 0);
  assert.deepStrictEqual(supervisor.controlState().running, false);
  assert.deepStrictEqual(supervisor.controlState().owned, false);
});
