...
windows = createWindowLifecycle({
  ...
  // Late-bound: the supervisor is constructed first, and the tray must read
  // gateway state at menu-build time rather than capture a boot-time snapshot.
  gatewayControls: {
    state: () => gateway.controlState(),
    run: (action) => {
      void gateway.runControlAction(action);
    },
  },
});

// The tray's gateway status label is only honest if every ownership and
// liveness transition redraws it, including a child that died on its own.
gateway.observeState(() => windows.refreshTrayMenu());
