...
function createWindowLifecycle(options) {
  const {
    ...
    requestQuit,
    connectWindow,
    // Optional. Absent, the tray renders exactly the pre-gateway-controls menu,
    // so nothing here depends on a supervisor being wired in.
    gatewayControls = null,
  ...
  let mainWindow = null;
  let tray = null;
  // Assigned by createTray; a no-op before the tray exists.
  let refreshTrayMenu = () => {};
  ...
  function createTray() {
    ...
    // Rebuilt rather than mutated on every gateway transition: Electron's menu
    // items are immutable once built, so a live status label needs a new
    // template. Without gatewayControls this runs exactly once.
    refreshTrayMenu = () => {
      ...
      if (gatewayControls) {
        const { running, owned, operation } = gatewayControls.state();
        const status = operation ? `${operation}…` : running ? "Running" : "Stopped";
        items.push(
          { label: `Gateway: ${status}`, enabled: false },
          { label: "Start Gateway", enabled: !running, click: () => gatewayControls.run("start") },
          { label: "Stop Gateway", enabled: owned, click: () => gatewayControls.run("stop") },
          { label: "Restart Gateway", enabled: owned, click: () => gatewayControls.run("restart") },
          { type: "separator" },
        );
      }
      items.push(
